...
#' @param LD.path Path to the directory where linkage disequilibrium (LD) information is stored. see note below.
#' @param bim.path Path to the directory where the decompressed bimfile.zip file is stored. see note below.
...
#' @param piece The piece of the genome to which the region belongs. The whole genome is divided into 2,468 smaller, semi-independent blocks, each defined by LD calculated by Plink. The SNP information in each local region is included in the data set: "HDLL_LOC_snps.RData".
#' @param alpha The significance level used to compute the cut-off value 'c' for the likelihood-based confidence interval (LCI). The cut-off 'c' is calculated using the formula: c = exp(-qchisq(1 - alpha, 1) / 2). For example, when alpha = 0.05, it corresponds to a 95 percent likelihood-based confidence interval.
#' @note Users can download the precomputed eigenvalues and eigenvectors of LD correlation matrices for European ancestry population. The download link can be found at https://doi.org/10.5281/zenodo.14825987 (LDfile.zip, bimfile.zip)
...
#' @import dplyr
#' @export
HDL.L <- function(gwas1.df, gwas2.df, Trait1name, Trait2name, LD.path, bim.path, Nref = 335272, N0, chr, piece, output.file = "", eigen.cut = 0.99, intercept.output = FALSE, fill.missing.N = NULL, lim = exp(-18), alpha = 0.05) {
  if (output.file != "") {
  ...
  llfun.gcov.part.2 = function(param, h11, h22, rho12, M, N1, N2, N0, Nref, lam1, lam2, bstar1, bstar2, lim) {
    h12 = param[1]
    int = param[2]
    ## sample fractions
    p1 = N0 / N1
    ...
    lam11 =   h11[1] / M * lam1 ^ 2 - h11[1] * lam1 / Nref + h11[2] * lam1 /
      N1
    lam11 = ifelse(lam11 < lim, lim, lam11)
    lam22 = h22[1] / M * lam2 ^ 2 - h22[1] * lam2 / Nref + h22[2] * lam2 /
      N2
  ...
  llfun0.gcov.part.2 = function(int, h11, h22, rho12, M, N1, N2, N0, Nref, lam1, lam2, bstar1, bstar2, lim) {
    h12 = 0
    ## sample fractions
    p1 = N0 / N1
    p2 = N0 / N2
  ...
  find_LD_rda_file <- function(LD.files, chr, piece) {
    pattern1 <- paste0("ukb_chr", chr, "\\.", piece, "_.*_LDSVD\\.rda$")
  ...
  LD_bim_file <- bim.files[grep(x = bim.files,
                                pattern = paste0("ukb_", chr, ".", piece, "[\\._].*bim"))]
  if (length(LD_bim_file) == 0) {
    LD_bim_file <- bim.files[grep(x = bim.files,
                                  pattern = paste0("ukb_chr", chr, ".", piece, "[\\._].*bim"))]
  ...
  if (!("Z" %in% colnames(gwas1.df))) {
    if (("b" %in% colnames(gwas1.df)) &&
        ("se" %in% colnames(gwas1.df))) {
      if (abs(median(gwas1.df$b) - 1) < 0.1) {
        cat("Taking log(b) in GWAS 1 because b is likely to be OR in stead of log(OR). \n")
        if (output.file != "") {
          cat(
            "Taking log(b) in GWAS 1 because b is likely to be OR in stead of log(OR). \n",
            file = output.file,
            append = T
          )
        }
  ...
  if (!("Z" %in% colnames(gwas2.df))) {
    if (("b" %in% colnames(gwas2.df)) &&
        ("se" %in% colnames(gwas2.df))) {
      if (abs(median(gwas2.df$b) - 1) < 0.1) {
        cat("Taking log(b) in GWAS 2 because b is likely to be OR in stead of log(OR). \n")
        if (output.file != "") {
          cat(
            "Taking log(b) in GWAS 2 because b is likely to be OR in stead of log(OR). \n",
            file = output.file,
            append = T
          )
        }
  ...
  cat(k1.0 - k1,
      "SNPs were removed in GWAS 1 due to missing N or missing test statistic.",
      " \n")
  cat(k2.0 - k2,
      "SNPs were removed in GWAS 2 due to missing N or missing test statistic.",
      " \n")
  if (output.file != "") {
    cat(
      k1.0 - k1,
  ...
  rho12 <- suppressWarnings(
    inner_join(
      gwas1.df %>% select(SNP, Z) %>% filter(abs(Z) < 8),
      gwas2.df %>% select(SNP, Z) %>% filter(abs(Z) < 8),
      by = "SNP"
  ...
  if (any(duplicated(gwas1.df.subset$SNP)) == TRUE) {
    ...
    SNP_A1_A2.valid <- c(
      paste(
        snps.ref.df.duplicated$id,
        snps.ref.df.duplicated$A1,
        snps.ref.df.duplicated$A2,
        sep = "_"
      ),
      paste(
        snps.ref.df.duplicated$id,
  ...
  if (N0 > 0)
    h12v = sqrt(h11v * h22v) + (h12.ols[2] * LDsc / M + p1 * p2 * rho12 / N0) ^
    2
  if (N0 == 0)
  ...
  for (start_val_1 in starting_values_1) {
    for (start_val_2 in starting_values_2) {
      for (n_deps in ndeps_values) {
        ...
        opt = optim(
          c(start_val_1, start_val_2),
          llfun,
          N = N1,
          Nref = Nref,
          lam = unlist(lam.v.cut),
          bstar = unlist(bstar1.v.cut),
          M = M.ref,
          lim = lim,
          method = 'L-BFGS-B',
          lower = c(0, 0),
        ...
        if (opt$convergence == 0 &&
            opt0$convergence == 0 &&
            opt$value > best_value && opt$value > opt0$value) {
  ...
  if (h11 <= 0 | h22 <= 0) {
    ...
    starting_values_1 <- c(h12.wls[2], 0, -sqrt(h11 * h22) * 0.5, sqrt(h11 *
                                                                         h22) * 0.5)
      ...
      for (start_val_2 in starting_values_2) {
        for (n_deps in ndeps_values) {
          ...
          opt <- optim(
            ...
            N0 = N0,
            Nref = Nref,
            lam1 = unlist(lam.v.use),
            lam2 = unlist(lam.v.use),
            bstar1 = unlist(bstar1.v.use),
            bstar2 = unlist(bstar2.v.use),
            lim = lim,
            method = 'L-BFGS-B',
            lower = c(-sqrt(h11 * h22), -20),
            upper = c(sqrt(h11 * h22), 20),
            control = list(
              factr = 1e-8,
              maxit = 1000,
              trace = 0,
  ...
  if (h11.LCI == 0 | h22.LCI == 0) {
    ...
    h12_val = seq(-1 * sqrt(h11.LCI * h22.LCI),
                  1 * sqrt(h11.LCI * h22.LCI),
                  length = 10000)
  ...
  cat("\n")
  cat("Point estimates: \n")
  cat("Heritability of phenotype 1: ", output(h11), "\n")
  cat("Heritability of phenotype 2: ", output(h22), "\n")
  cat("Genetic Covariance: ", output(h12), "\n")
  cat("Genetic Correlation: ", output(rg), "\n")
  if (h11 == 0 | h22 == 0) {
    cat(
