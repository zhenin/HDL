...
#' 
#' The function returns estimation and standard error of genetic correlation between two traits based on GWAS summary statistics. 
#' 
...
#' @param output.file The file where the log should be saved. If not given, the log will not be saved.
#' 
#' @note Users can download the eigenvalues and eigenvectors of LD correlation matrices from link. 
#' These eigens of LD are from eigen-decomposition results based on 307,519 QCed UK Biobank Axiom Array SNPs. 
#' 336,000 ethnically British individuals in UK Biobank are used to compute LD.
#' 
#' 
#' @return A list is returned with:
#' \itemize{
#' \item{rg }{The estimation of genetic correlation.}
#' \item{rg.se }{The standard error of estimated genetic correlation.}
#' }
#' 
#' @author Zheng Ning
#' 
...
function(gwas1.df, gwas2.df, LD.path, Nref = 336000, N0 = min(gwas1.df$N, gwas2.df$N), output.file = ""){
  rownames(gwas1.df) <- gwas1.df$rsid
  rownames(gwas2.df) <- gwas2.df$rsid
  ...
  for(chr in 1:22){
    ...
    for(piece in 1:k){
      ...
      A2.gwas <- gwas1.df.subset[, "A2"]
      
      idx <- A2.gwas == A2.ref
      bhat1 <- bhat1*(2*as.numeric(idx)-1)
      bhat2 <- bhat2*(2*as.numeric(idx)-1)
  ...
  ## rg is sensible
  if(abs(rg) <1){
    cat("rg is within (-1,1). Continuing computing standard error. \n")
    set.seed(510)
    ...
    return(list(rg = rg, rg.se = rg.se))
  } else{
    ## rg is not sensible, switch to full likelihood for rg estimation, slower
    cat("rg beyonds (-1,1). Switching to full likelihood. \n")
